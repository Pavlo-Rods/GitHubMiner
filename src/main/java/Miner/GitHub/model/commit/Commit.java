...
public class Commit {
    ...
    public static Commit ofFormat(CommitSearch cs){
        Commits c = cs.getCommit();
        Author a = c.getAuthor();
        Committer ct = c.getCommitter();

        return new Commit(cs.getSha(), null, c.getMessage(), a.getName(), a.getEmail(),
                a.getDate(), ct.getName(), ct.getEmail(), ct.getDate(),
                cs.getHtmlUrl());
    }
